...
namespace ZCJT.BLL
{
    public class SysUserBLL : BaseBLL, ISysUserBLL
    {
        ...
        private List<SysUserModel> CreateModelList(ref IQueryable<SysUser> queryData)
        {
            ...
            foreach (var sysUserModel in modelList)
            {
                sysUserModel.RoleNames = GetRefSysRole(sysUserModel.Id);
            }
            return modelList;
        }
        ...
        {
            string roleName = "";
            var roleList = m_Rep.GetRoleByUserId(db, userId);
            if (roleList != null)
            {
                roleName = Enumerable.Aggregate(roleList.Where(p=>p.flag==userId), roleName, (current, role) => current + ("[" + role.Name + "] "));
            }
            return roleName;
        }
