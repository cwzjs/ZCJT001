...
namespace ZCJT.BLL
{
    public class SysRoleBLL : BaseBLL, ISysRoleBLL
    {
        ...
        public List<SysRoleModel> GetList(ref GridPager pager, string queryStr)
        {
            ...
            {
                queryData = m_Rep.GetList(db).Where(a => a.Name.Contains(queryStr) || a.Description.Contains(queryStr));
            }
            else
            {
        ...
        private List<SysRoleModel> CreateModelList(ref IQueryable<SysRole> queryData)
        {
            ...
            var modelList= new List<SysRoleModel>();
            ...
            foreach (var qd in queryData)
            {
                var sysRoleModel = new SysRoleModel
                {
                    ...
                    Name = qd.Name,
                    CreateTime = qd.CreateTime,
                    CreatePerson = qd.CreatePerson,
                    UserName = GetRefSysUser(qd.Id)
                };
                modelList.Add(sysRoleModel);
            }
            return modelList;
        }
        ...
        public bool Create(ref ValidationErrors errors, SysRoleModel model)
        {
            try
            {
                ...
                {
                    db.P_Sys_InsertSysRight();//在新增角色组的时候，对该角色组进行模块false授权
                    return true;
                }
                else
                {
