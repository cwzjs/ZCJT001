...
namespace ZCJT.Web.Controllers
{
    public class SysUserController : BaseController
    {
        ...
        public JsonResult Edit(SysUserModel model)
        {
            if (model != null && ModelState.IsValid)
            {
                var oldmodel = m_BLL.GetById(model.Id);
        ...
        public JsonResult UpdatePwdSet(string oldPwd, string newPwd)
        {
            SysUserModel user = m_BLL.GetById(CurrentAccount.Id);

            if (user.Password == ValueConvert.MD5(oldPwd))
            {
                user.Password = ValueConvert.MD5(newPwd);
                if (m_BLL.Edit(ref errors, user))
                {
                    LogHandler.WriteServiceLog(GetUserId(), "Id" + user.Id + ",UserName" + user.UserName, "成功", "修改密码", "SysUser");
                    return Json(JsonHandler.CreateMessage(1, Suggestion.SetSucceed));
                }
                else
                {
                    string ErrorCol = errors.Error;
                    LogHandler.WriteServiceLog(GetUserId(), "Id" + user.Id + ",UserName" + user.UserName + "," + ErrorCol, "失败", "修改密码", "SysUser");
                    return Json(JsonHandler.CreateMessage(0, Suggestion.SetFail + ErrorCol));
                }
            }
            else
            { 
                return Json(JsonHandler.CreateMessage(0, Suggestion.SetFail + "原密码错误，请重新输入"), JsonRequestBehavior.AllowGet);
            }
